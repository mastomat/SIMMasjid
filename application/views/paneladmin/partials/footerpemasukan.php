						...
						<div class="card-body">
							<div class="row">
								<div class="col-md-6">
									...
									<div class="form-group">
										<label>Jenis Pemasukan <span>*</span></label>

										<select id="jenis" name="jenis" class="select2 form-control" required="required">
							...
							<div class="row">
								<div class="col-md-6">
									...
									<div class="form-group">
										...
										<select id="ejenis" name="ejenis" class="select2 form-control" required="required">
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title" id="myModalLabel">Hapus Pemasukan</h4>
			</div>
			<form>
				<div class="modal-body">
					...
					<div class="alert alert-warning">
						<p>Apakah Anda yakin Menghapus Data Pemasukan ini?</p>
					</div>
...
<script>
	$('#jenismasukan').change(function() {
		$('#datamasuk').DataTable().ajax.reload();
	});
	...
	$(document).ready(function() {
		$('#datamasuk').DataTable({
			...
			"ajax": {
				url: "<?php echo base_url() ?>k_pemasukan/getdata",
				type: 'GET',
				data() {
					return {
						jenis: $('#jenismasukan').find(":selected").val(),
					};
				},
		...
		function totalpemasukan() {
			...
			$.ajax({
				...
				success: function(data) {
					$('#totalpemasukan').html(data.total)
		...
		$.validator.setDefaults({
			submitHandler: function() {
				...
				$.ajax({
					type: "POST",
					url: "<?php echo base_url() ?>k_pemasukan/simpan",
					data: {
						'tanggal': tanggal,
						'jumlah': jumlah,
						'jenis': jenis,
		...
		$('#datamasuk').on('click', '.bhapus', function() {
		...
		$('#btn_hapus').on('click', function() {
			...
			$.ajax({
				type: "POST",
				url: "<?php echo base_url() ?>k_pemasukan/hapus",
				data: {
					id: kode
				},
				success: function(data) {
		...
		//showedit
		$('#datamasuk').on('click', '.bedit', function() {
			var id = $(this).attr('data');
			$.ajax({
				type: "GET",
				url: "<?php echo base_url() ?>k_pemasukan/showedit",
				dataType: "JSON",
				data: {
					id: id
				},
				success: function(data) {
					$.each(data, function(jumlah, tanggal, jenis, namajenis,
						ljenis) {
						...
						var html = '';
						var i;
						html += '<option value=' + data.jenis + ' >' + data
							.namajenis + '</option>';
						for (i = 0; i < data.ljenis.length; i++) {
							html += '<option value=' + data.ljenis[i].jenis_id + ' >' +
								data.ljenis[i].jenis_nama + '</option>';
						}
						$('#ejenis').html(html)
					});
		...
		$.validator.setDefaults({
			submitHandler: function() {
				...
				var jenis = $('#ejenis').find(":selected").val();

				console.log(id);
				$.ajax({
					type: "POST",
					url: "<?php echo base_url() ?>k_pemasukan/simpan_edit",
					data: {
						'jumlah': jumlah,
						'tanggal': tanggal,
						'jenis': jenis,
